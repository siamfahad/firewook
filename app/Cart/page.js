...
const CartPage = () => {
  const { cartItems, handleQuantityChange, handleRemoveItem, handleClearCart } = useCart();
        ...
        {cartItems.length === 0 ? (
          <div className="text-center py-12">
            ...
            <Link href="/Menu" passHref>
              <button
                className="bg-red-600 hover:bg-red-500 text-white font-bold py-3 px-6 rounded-full text-lg transition-colors duration-300 shadow-md"
              >
                Start Shopping
              </button>
            </Link>
          ...
          <div>
            <ul className="space-y-6 mb-8">
              {cartItems.map((item) => (
                <li key={item.id} className="flex items-center space-x-4 bg-gray-700 p-4 rounded-lg shadow-md">
                  ...
                  <div className="text-right">
                    <p className="text-lg font-bold text-white mb-2">${(item.price * item.quantity).toFixed(2)}</p>
                    <button onClick={() => handleRemoveItem(item.id)} className="text-sm text-gray-400 hover:text-red-400 transition-colors">
                      Remove
                    </button>
                  </div>
                </li>
            ...
            <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4 mt-8">
              <Link href="/Menu" passHref>
                <button
                  className="flex-1 text-center bg-gray-600 hover:bg-gray-500 text-white font-bold py-3 px-6 rounded-full text-lg transition-colors duration-300"
                >
                  Continue Shopping
                </button>
              </Link>
